...
use vulkano_taskgraph::{
    ClearValues, Id, QueueFamilyType, Task, TaskContext, TaskResult,
    command_buffer::{
...
//const TRANSFER_GRANULARITY_X: u32 = 1280;
//const TRANSFER_GRANULARITY_Y: u32 = 720;
const TRANSFER_GRANULARITY_X: u32 = 3200;
const TRANSFER_GRANULARITY_Y: u32 = 1200;
...
impl App {
    fn init_camera(tx: Sender<Vec<u8>>) -> io::Result<()> {
        let path = "/dev/video4";
        ...
        let mut format: v4l::Format;
        let params: v4l::video::capture::Parameters;

        let dev = RwLock::new(v4l::device::Device::with_path(path)?);
        {
            ...
            if format.fourcc != FourCC::new(b"RGB3") {
                ...
                if format.fourcc != FourCC::new(b"MJPG") {
                    return Err(io::Error::new(
                         io::ErrorKind::Other,
                         "neither RGB3 nor MJPG supported by the device, but required by this example!".to_string(),
                     ));
                }
            }
        }
        ...
        thread::spawn(move || {
            loop {
                ...
                if let Ok(data) = data {
                    ...
                    println!("[!] skip frame that could not be decoded");
                }
            }
        });
        Ok(())
    ...
    fn new(event_loop: &EventLoop<()>) -> Self {
        ...
        let transfer_family_index = physical_device
            ...
            .filter(|(_, q)| {
                ...
                && q.min_image_transfer_granularity[0..2]
                    .iter()
                    .all(|&g| TRANSFER_GRANULARITY_X % g == 0)
            })
            ...
            resources
                .create_image(
                    ImageCreateInfo {
                        ...
                        extent: [TRANSFER_GRANULARITY_X, TRANSFER_GRANULARITY_Y, 1],
        ...
        App::init_camera(channel).unwrap();
                ...
                .create_window(
                    Window::default_attributes()
                        ...
                        .with_inner_size(PhysicalSize::new(
                            TRANSFER_GRANULARITY_X,
                            TRANSFER_GRANULARITY_Y,
                        )),
                )
                .unwrap(),
        );
    ...
    vulkano_shaders::shader! {
        ...
        src: r"
            ...
            void main() {
                ...
                tex_coords = position * vec2(0.5) + vec2(0.5);
            }
        ",
    }
}
    ...
    unsafe fn execute(
        ...
        *tcx.write_buffer(self.uniform_buffer_id, ..)? = vs::Data {
            transform: {
                ...
                Mat4::from_rotation_z(0.0).to_cols_array_2d()
            },
        };
...
fn run_worker(
        ...
        resources
            .create_buffer(
                ...
                DeviceLayout::from_size_alignment(
                    TRANSFER_GRANULARITY_X as DeviceSize * TRANSFER_GRANULARITY_Y as DeviceSize * 4,
                    1,
                )
                .unwrap(),
            )
    ...
    thread::spawn(move || {
        ...
        while let Ok(image) = channel.recv() {
            ...
            unsafe { task_graph.execute(resource_map, &image, || {}) }.unwrap();
...
impl Task for UploadTask {
    ...
    unsafe fn execute(
        ...
        tcx.write_buffer::<[_]>(self.front_staging_buffer_id, ..)?
            ...
            .write_all(&data_to_render.chunks_exact(3).flat_map(|i| [i[0], i[1], i[2], 255]).collect::<Vec<u8>>()).unwrap()
            //.write_all((0..1280 * 720).map([255, 255, 255, 255]))
           ;
        ...
        cbf.copy_buffer_to_image(&CopyBufferToImageInfo {
            ...
            regions: &[BufferImageCopy {
                ...
                image_extent: [TRANSFER_GRANULARITY_X, TRANSFER_GRANULARITY_Y, 1],
                ..Default::default()
            }],
            ..Default::default()
        })?;
        //TODO: check how to use back staging buffer
        cbf.copy_buffer_to_image(&CopyBufferToImageInfo {
